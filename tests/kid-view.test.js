...
T('이름은 아이 색', await p.$eval('.kid .kn',e=>getComputedStyle(e).color==='rgb(142, 42, 85)'));
let r=await rows();
T('16,300 = 1만·5천·1천·100×3', r.length===4 && r[0].t==='1만'&&r[0].n===1 && r[1].t==='5천' && r[2].t==='1천' && r[3].t==='100'&&r[3].n===3);
...
T('84,950 = 6줄', r.length===6 && r.map(x=>x.t).join()==='5만,1만,1천,500,100,50');
...
await p.click('.balbtn'); await p.waitForTimeout(300); r=await rows();
T('30만 = 5만 5장 + ×6', r.length===1 && r[0].n===5 && r[0].x==='×6');
await p.click('.kid'); await p.waitForTimeout(150);
...
T('노랑 아이 이름색', await p.$eval('.kid .kn',e=>getComputedStyle(e).color==='rgb(122, 90, 14)'));
